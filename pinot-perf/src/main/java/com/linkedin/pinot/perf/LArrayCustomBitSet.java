 ...
 */
public final class LArrayCustomBitSet {
  ...
  public LArrayCustomBitSet(File file){
    byteArray = LArrayJ.loadLByteArrayFrom(file);    
    nrBytes = byteArray.length();
  }

  static int bitMasks[] = new int[32];
  private LByteArray byteArray;
  static {
  ...
  public int readInt(long startBitIndex, long endBitIndex) {
    ...
    if (bitLength < 16 && endBitIndex + 32 < nrBytes * 8L) {
      ...
      int intValue = byteArray.getInt(bytePosition);
      ...
      long number = 0;
      while (true) {
        number |= (byteArray.getByte(bytePosition)) & 0xFF;
        i++;
        bytePosition++;
  ...
  public long findNthBitSetAfter(long startBitIndex, int n) {
    ...
    int currentByte = (byteArray.getByte(bytePosition) << bitPosition) & 0xFF;
    ...
    while (numberOfBitsOnInCurrentByte <= numberOfBitsToSkip) {
      ...
      currentByte = byteArray.getByte(bytePosition) & 0xFF;
  ...
  public long nextSetBit(long index) {
    ...
    while (currentByte == 0) {
      ...
      currentByte = byteArray.getByte(bytePosition + bytesSkipped) & 0xFF;
    }
  ...
  public boolean isBitSet(long index) {
    ...
    byte b = byteArray.getByte(byteToCheck);
